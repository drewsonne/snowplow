...
object ApiRequestEnrichmentConfig extends ParseableEnrichment {
  ...
  val supportedSchema =
    SchemaCriterion("com.snowplowanalytics.snowplow.enrichments", "api_request_enrichment_config", "jsonschema", 1, 0, 0)
...
case class ApiRequestEnrichment(inputs: List[Input], api: HttpApi, outputs: List[Output], cache: Cache) extends Enrichment {
  ...
  def lookup(event: EnrichedEvent,
    ...
    val templateContext = Input.buildTemplateContext(inputs, event, derivedContexts, jsonCustomContexts, jsonUnstructEvent)
