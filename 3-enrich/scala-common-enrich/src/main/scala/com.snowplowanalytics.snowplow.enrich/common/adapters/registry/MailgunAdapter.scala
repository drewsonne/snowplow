...
def toRawEvents(payload: CollectorPayload)(implicit resolver: Resolver): ValidatedRawEvents = {
  ...
  /**
   * Converts dash separated strings with uppercase to snake_case.
   * It does not separte camelCased strings
   *
   * @param dashWithUpper string like "X-Mailgun-Sid"
   * @return string like "x_mailgun_sid"
   */
  private def snakeCase(dashWithUpper: String) = dashWithUpper.replace("-", "_").map(_.toLower)
 /**
  ...
  */
  private def mutateMailgunEvent(json: JValue): JValue = {
    val jsonSnakeCase: JValue = json.mapField { case (fieldName, jv) => (snakeCase(fieldName), jv)}
    val dropFields: JObject = jsonSnakeCase.filterField({case (name: String, _) => !(name == "body_plain" || name == "attachment_count")})
    (jsonSnakeCase \ "attachment_count").extractOpt[String].map(ac =>  ("attachment_count" -> JInt(ac.toInt)) ~ dropFields).getOrElse(dropFields)
  }
