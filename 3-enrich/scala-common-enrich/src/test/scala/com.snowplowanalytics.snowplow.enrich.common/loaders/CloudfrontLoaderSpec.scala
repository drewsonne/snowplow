...
class CloudfrontLoaderSpec extends Specification with DataTables with ValidationMatchers with ScalaCheck {
  ...
  def e1 =
    "SPEC NAME"             || "DATE"       | "TIME"         | "EXP. DATETIME" |
      ...
      "Valid without ms #2" !! "1980-04-01" ! "21:20:04"     ! DateTime.parse("1980-04-01T21:20:04+00:00") |> { (_, date, time, expected) =>
      {
  ...
  def e3 =
    "SPEC NAME"                   || "URI"                                              | "EXP. URI" |
      ...
      "URI without trailing % #2" !! "http://bbc.co.uk"                                 ! "http://bbc.co.uk" |> { (_, uri, expected) =>
      {
  ...
  def e7 =
    check { (raw: String) =>
      CloudfrontLoader.toCollectorPayload(raw) must beFailing(NonEmptyList("Line does not match CloudFront header or data row formats"))
    }
}
