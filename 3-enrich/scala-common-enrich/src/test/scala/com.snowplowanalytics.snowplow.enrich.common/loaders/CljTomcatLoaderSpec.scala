...
class CljTomcatLoaderSpec extends Specification with DataTables with ValidationMatchers with ScalaCheck {
      ...
      (_, raw, version, payload, contentType, body, timestamp, userAgent, refererUri) =>
        {
          ...
          val canonicalEvent = CljTomcatLoader.toCollectorPayload(raw)
  ...
  def e2 = {
    ...
    actual must beFailing(NonEmptyList("Operation must be POST, not GET, if request content type and/or body are provided"))
  }
  ...
  def e3 = {
    ...
    actual must beFailing(
      NonEmptyList("Request path /com.sendgrid-api-v3 does not match (/)vendor/version(/) pattern nor is a legacy /i(ce.png) request"))
  }
  ...
  def e4 =
    check { (raw: String) =>
      CljTomcatLoader.toCollectorPayload(raw) must beFailing(NonEmptyList("Line does not match raw event format for Clojure Collector"))
    }
}
