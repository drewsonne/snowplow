...
class IgluAdapterSpec extends Specification with DataTables with ValidationMatchers with ScalaCheck {
  ...
  def e1 = {
    ...
    actual must beSuccessful(
      NonEmptyList(RawEvent(Shared.api, Expected.static ++ Map("ue_pr" -> expectedJson), None, Shared.cfSource, Shared.context)))
  }
  ...
  def e4 = {
    ...
    actual must beSuccessful(NonEmptyList(
      RawEvent(Shared.api, Expected.staticNoPlatform ++ Map("p" -> "mob", "ue_pr" -> expectedJson), None, Shared.cfSource, Shared.context)))
  }
  ...
  def e13 = {
    ...
    val jsonStr = """{"schema":"iglu:com.acme/campaign/jsonschema/1-0-1", "data":{"some_param":"foo"}}"""
    val payload =
      CollectorPayload(Shared.api, params, "application/xxx-url-form-encoded".some, jsonStr.some, Shared.cljSource, Shared.context)
    val actual = IgluAdapter.toRawEvents(payload)
  ...
  def e15 = {
    ...
    val payload =
      CollectorPayload(Shared.api, params, "application/x-www-form-urlencoded".some, formBodyStr.some, Shared.cljSource, Shared.context)
    val actual = IgluAdapter.toRawEvents(payload)
