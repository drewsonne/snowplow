...
// Snowplow
import loaders.{TsvLoader, CollectorApi, CollectorSource, CollectorContext, CollectorPayload}
...
class CloudfrontAccessLogAdapterSpec extends Specification with DataTables with ValidationMatchers with ScalaCheck {
  ...
  toRawEvents should return a Validation Failure if the line is the wrong length           $e7
  toRawEvents should return a Validation Failure if the line contains an unparseable field $e8
  """
  ...
  object Shared {
    ...
    val context =
      CollectorContext(DateTime.parse("2013-10-07T23:35:30.000Z").some, "255.255.255.255".some, singleEncodedUa.some, None, Nil, None)
  }
  ...
  def e1 = {
    ...
    actual must beSuccessful(
      Some(
        Success(NonEmptyList(RawEvent(Shared.api, Expected.static ++ Map("ue_pr" -> expectedJson), None, Shared.source, Shared.context)))))
  }
  ...
  def e8 = {
    val params = toNameValuePairs()
    val payload =
      CollectorPayload(Shared.api, params, None, "a\tb\tc\td\te\tf\tg\th\ti\t$url\tk\t$doubleEncodedQs".some, Shared.source, Shared.context)
    val actual = CloudfrontAccessLogAdapter.WebDistribution.toRawEvents(payload)
