...
class AdapterSpec extends Specification with DataTables with ValidationMatchers with ScalaCheck {
  ...
  def e5 =
    "SPEC NAME"                  || "SCHEMA TYPE" | "EXPECTED OUTPUT" |
      ...
      "Failing, bad type passed" !! Some("bad")   ! "Adapter event failed: type parameter [bad] not recognized" |> { (_, et, expected) =>
  ...
  def e7 = {
    ...
    val validatedRawEventsList =
      List(Success(rawEvent), Failure(NonEmptyList("This is a failure string-1")), Failure(NonEmptyList("This is a failure string-2")))
  ...
  def e9 =
    "SPEC NAME"                || "JSON" | "EXPECTED OUTPUT" |
      ...
      "Change nested values" !! """{"ts":1415709559,"nested":{"ts":1415700000}}""" ! JObject(
        List(("ts", JString("2014-11-11T12:39:19.000Z")), ("nested", JObject(List(("ts", JString("2014-11-11T10:00:00.000Z"))))))) |
  ...
  def e10 =
    "SPEC NAME"                    || "JSON" | "EXPECTED OUTPUT" |
      ...
      BaseAdapter.cleanupJsonEventValues(parse(json), ("an_event", "type").some, "ts", _ * 1000) mustEqual expected
    }
