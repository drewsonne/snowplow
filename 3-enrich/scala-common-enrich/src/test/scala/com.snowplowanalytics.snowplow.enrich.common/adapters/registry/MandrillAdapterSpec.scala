...
class MandrillAdapterSpec extends Specification with DataTables with ValidationMatchers with ScalaCheck {
  ...
  def e6 = {
    ...
    MandrillAdapter.toRawEvents(payload) must beFailing(NonEmptyList("Request body is empty: no Mandrill events to process"))
  }
  ...
  def e7 = {
    ...
    MandrillAdapter.toRawEvents(payload) must beFailing(
      NonEmptyList("Request body provided but content type empty, expected application/x-www-form-urlencoded for Mandrill"))
  }
